...
(function() {
    ...
    if (window.location.pathname === '/all') {
        iframeConfigs.forEach(config => {
            ...
            processDocument(iframe.contentDocument, `iframe ${config.id}`);
        });
    } else {
        processDocument(document, 'main document');
    }
})();
"""
    ...
    def export_pdf(self, results=None):
        ...
        if is_iframe_mode and self.iframe_index < len(iframe_configs):
            ...
            (function() {
                ...
                ['#data-table', '.data-table'].forEach(selector => {
            ...
            self.iframe_index += 1
            QTimer.singleShot(2000, lambda: self.export_pdf(results))
        else:
